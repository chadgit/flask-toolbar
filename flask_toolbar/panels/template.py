...
from flask_toolbar import module
from flask_toolbar.panels import ToolbarPanel

_ = lambda x: x


class TemplateToolbarPanel(ToolbarPanel):
    """
...
def is_editor_enabled():
    return current_app.config.get('TB_TEMPLATE_EDITOR_ENABLED')
...
def template_editor(key):
    ...
    templates = [t['template'] for t in
                 TemplateToolbarPanel.get_cache_for_key(key)]
    return g.toolbar.render('panels/template_editor.html', {
        'static_path': url_for('_toolbar.static', filename=''),
        'request': request,
        'templates': [
...
def save_template(key):
    require_enabled()
    template = TemplateToolbarPanel.get_cache_for_key(key)[0]['template']
...
def template_preview(key):
    require_enabled()
    context = TemplateToolbarPanel.get_cache_for_key(key)[0]['context']
